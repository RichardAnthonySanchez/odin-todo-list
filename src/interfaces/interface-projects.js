...
const ProjectsInterface = (function () {
  function displayProjectTitle(selectedProject) {
    ...
    if (projects.length === 0) {
      ...
      const { title, subTitle } =
        ProjectsModel.getProjectTitle(selectedProject);
  ...
  function createNewProject(newProjectName) {
    ...
    const projectObject = ProjectsModel.createProjectObject(
      newProjectName,
      projects,
    );
  ...
  function updateProjectNameInterface(updatedName) {
    ...
    projectObject = ProjectsModel.modfiyProjectObjectByName(
      projectObject,
      updatedName,
    );
  ...
  function projectManagerInterface() {
    ...
    if (projectActionIndex === "1") {
      projectObject = ProjectsModel.createProjectObject(
        projectContent,
        projects,
      );
      ProjectsModel.addProject(projectObject);
      ...
      StateInterface.setStateInterface({
