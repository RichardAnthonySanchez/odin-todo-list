...
const TodosInterface = (function () {
  ...
  function updateTodoDescription(todo, newPropertyValue) {
    const updatedTodo = TodosModel.updateTodoDescription(
      todo,
      newPropertyValue,
    );
    TodosModel.updateTodo(updatedTodo);
  ...
  function changeTodoPropertyInterface(newPriorityValue) {
    // this method should  be named priority not property
    const state = TodosModel.getState();
    const selectedTodoIndex = state.selectedTodo;
    const todos = TodosModel.getTodos();
    const todoObject = TodosModel.getTodoFromId(selectedTodoIndex, todos);
    const updatedTodo = TodosModel.updateTodoPriority(
      todoObject,
      newPriorityValue,
    );
  ...
  function changeTodoProjectInterface(newProjectName) {
    ...
    const splicedProject = ProjectsModel.removeTodoFromProject(
      selectedTodo,
      selectedProject,
    ); //debug every thing under this within the method
    ...
    const updatedProject = ProjectsModel.addTodoToProject(
      selectedTodo,
      newProjectName,
    );
