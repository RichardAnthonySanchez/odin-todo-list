...
function component() {
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "Enter" && e.target.id === "add-task-input") {
      ...
      const selectedTodo =
        TodosInterface.getTodoFromIdInterface(selectedTodoId);
  ...
  document.addEventListener("click", function (e) {
    ...
    if (e.target.id === "refresh-todos") {
      ...
      (!nav.contains(e.target) && nav.classList.contains("active"))
    ) {
  ...
  function handleTaskClick(taskElement) {
    ...
    const selectedProject =
      ProjectsInterface.getProjectFromIdInterface(selectedProjectId);
