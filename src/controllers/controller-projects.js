...
const ProjectsController = (function () {
  ...
  function getNewProjectName() {
    let newProjectName = prompt(
      "Please type the desired name for your new project",
    );
  ...
  function mapProjects(projects) {
    const projectList = _.map(
      projects,
      (project) => `${project.id}: ${project.name} (${project.todos.length})`,
    ).join("\n");
    return projectList;
  }
  ...
  function projectSelector(projects) {
    ...
    const requestedId = prompt(
      `Please select a project by its id:\n${projectList}`,
    );
